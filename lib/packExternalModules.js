    ...
    if (moduleVersion) {
      ...
      try {
        ...
        if (!_.isEmpty(peerDependencies)) {
          this.serverless.cli.log(`Adding explicit peers for dependency ${module.external}`);
...
module.exports = {
  ...
  packExternalModules() {
    ...
    return BbPromise.try(() => Packagers.get.call(this, this.configuration.packager)).then(packager => {
      ...
      this.serverless.cli.log(`Fetch dependency graph from ${packageJsonPath}`);
        ...
        return BbPromise.fromCallback(cb => fse.pathExists(packageLockPath, cb))
          ...
          .then(() => {
            ...
            return packager
              ...
              .then(() => this.serverless.cli.log(`Package took [${_.now() - start} ms]`))
              .return(stats.stats);
          })
          .mapSeries(compileStats => {
            ...
            return BbPromise.try(() => {
              ...
              .tap(() => this.serverless.cli.log(`Copy modules: ${modulePath} [${_.now() - startCopy} ms]`))
              .then(() => {
                // Prune extraneous packages - removes not needed ones
                const startPrune = _.now();
                return packager
                  .prune(modulePath, this.configuration.packagerOptions)
                  .tap(() => this.serverless.cli.log(`Prune: ${modulePath} [${_.now() - startPrune} ms]`));
              })
              .then(() => {
                // Prune extraneous packages - removes not needed ones
                ...
                return packager
                  ...
                  .tap(() => this.serverless.cli.log(`Run scripts: ${modulePath} [${_.now() - startRunScripts} ms]`));
              });
          })
          .return();
      });
